"""
Author: Yixuan Su
Date: 2025/02/19 16:25
File: Open3D 计算点云的曲率密度参数.py
Description: 
...
if __name__ == '__main__':
    ...
    # ----------------------------使用伪颜色对点云进行渲染---------------------------
    sc_colors = plt.get_cmap('hot')(  # hot表示为热力图
        (sc_density - sc_density.min()) / (sc_density.max() - sc_density.min()))
    pcd.colors = o3d.utility.Vector3dVector(sc_colors[:, :3])
    # o3d.io.write_point_cloud("render.pcd", pcd)
    o3d.visualization.draw_geometries([pcd], window_name="点云曲率密度特征热力图渲染赋色",
